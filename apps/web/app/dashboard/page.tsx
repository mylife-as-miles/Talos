...
function MetricCard({
  ...
  return (
    <section className="rounded-lg border border-[#203037] bg-[linear-gradient(180deg,rgba(19,30,34,.86),rgba(10,18,22,.92))] p-5 shadow-[inset_0_1px_0_rgba(255,255,255,.035)]">
...
function InvestigationPanel() {
  return (
    <section className="rounded-lg border border-[#203037] bg-[linear-gradient(180deg,rgba(17,29,33,.88),rgba(9,17,21,.94))] p-6">
      ...
      <div className="mt-4 h-1.5 overflow-hidden rounded-full bg-[#223038]">
        <div className="h-full w-[91%] rounded-full bg-[#35d8e8]" />
      </div>
      ...
      <button className="mt-2 flex h-10 w-72 items-center justify-center gap-3 rounded-md border border-[#13566a] bg-[#0b2730] text-[14px] text-[#42e2ff]">
  ...
  return (
    <AppShell>
      <div className="flex items-start justify-between gap-6">
        <div>
          <h1 className="text-[34px] font-bold tracking-[-.03em] text-[#f4f7fb]">Talos Guardian Status</h1>
          <p className="mt-2 text-[17px] text-[#aab5bf]">Real-time overview of your runtime ecosystem</p>
        </div>
      ...
      <div className="mt-7 grid gap-3 xl:grid-cols-4">
        ...
        <section className="rounded-lg border border-[#203037] bg-[linear-gradient(180deg,rgba(19,30,34,.86),rgba(10,18,22,.92))] p-5">
          <div className="flex items-start justify-between">
            <div>
              <div className="text-[13px] uppercase tracking-[.09em] text-[#a8b3bd]">Latest Service</div>
              <div className="mt-5 text-[24px] font-semibold text-white">checkout-service</div>
              <div className="mt-4 text-[15px] text-[#9faab4]">nodejs <span className="mx-4">•</span> v2.14.1 <span className="mx-4">•</span> prod</div>
              <div className="mt-7 text-[14px] text-[#a2acb5]">2 min ago</div>
            </div>
      ...
      <div className="mt-3 grid gap-5 xl:grid-cols-[1.45fr_1fr]">
        <section className="rounded-lg border border-[#203037] bg-[linear-gradient(180deg,rgba(17,29,33,.88),rgba(9,17,21,.94))] p-5">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <button className="flex h-10 items-center gap-12 rounded-md border border-[#24343c] px-4 text-[14px] text-[#d6dee6]">Group by: Service <ChevronDown size={15} /></button>
              <button className="grid h-10 w-12 place-items-center rounded-md border border-[#24343c] text-[#c7d1d8]"><Filter size={20} /></button>
            </div>
          </div>
          <div className="mt-5 flex items-center gap-6 text-[14px] text-[#c1cad2]">
            <span className="flex items-center gap-2"><i className="h-3 w-3 rounded-sm bg-[#ef4d55]" />Critical</span>
            <span className="flex items-center gap-2"><i className="h-3 w-3 rounded-sm bg-[#f17b2d]" />High</span>
            <span className="flex items-center gap-2"><i className="h-3 w-3 rounded-sm bg-[#efb53f]" />Medium</span>
            <span className="flex items-center gap-2"><i className="h-3 w-3 rounded-sm bg-[#0a8ec1]" />Low</span>
            <span className="flex items-center gap-2"><i className="h-3 w-3 rounded-sm bg-[#5d6870]" />Info</span>
          </div>
          <RuntimeChart />
        </section>
        <InvestigationPanel />
      ...
      <section className="mt-2 rounded-lg border border-[#203037] bg-[linear-gradient(180deg,rgba(17,29,33,.88),rgba(9,17,21,.94))]">
        <div className="flex items-center justify-between px-5 py-4">
          ...
          <div className="flex gap-3">
            <button className="flex h-9 items-center gap-12 rounded-md border border-[#26363d] px-4 text-[14px] text-[#d6dee6]">All Services <ChevronDown size={15} /></button>
            <button className="flex h-9 items-center gap-12 rounded-md border border-[#26363d] px-4 text-[14px] text-[#d6dee6]">All Status <ChevronDown size={15} /></button>
            <button className="h-9 rounded-md border border-[#15556a] px-5 text-[14px] text-[#36e1ff]">View Full Incidents</button>
          </div>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[1180px] text-left text-[14px]">
            <thead className="border-y border-[#1f2d34] text-[13px] uppercase text-[#a8b3bd]">
              <tr>
                {["ID ↕", "Service", "Error", "Priority", "Status", "First Seen", "Last Seen", "Count", "Diagnosis Confidence", ""].map((heading) => <th key={heading} className="px-5 py-3 font-medium">{heading}</th>)}
              </tr>
            </thead>
            <tbody>
              {incidents.map(([id, service, runtime, error, priority, status, firstSeen, lastSeen, count, confidence, icon]) => (
                <tr key={id} className="border-b border-[#1c2930] text-[#cbd4dc]">
                  <td className="px-5 py-3 font-medium">{id}</td>
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-3">
                      <span className={`grid h-5 w-5 place-items-center rounded text-[10px] ${icon === "python" ? "bg-[#15395d] text-[#ffd544]" : icon === "go" ? "bg-[#083b48] text-[#42e6ff]" : icon === "java" ? "bg-[#3e2413] text-[#ff9a42]" : "bg-[#103b25] text-[#47e276]"}`}>{icon === "python" ? "Py" : icon === "go" ? "Go" : icon === "java" ? "J" : "N"}</span>
                      <span><span className="block text-[#e0e7ec]">{service}</span><span className="text-[12px] text-[#8d99a3]">{runtime}</span></span>
                    </div>
                  </td>
                  ...
                  <td className="px-5 py-3 text-[#aeb8c0]">{firstSeen}</td>
                  <td className="px-5 py-3 text-[#aeb8c0]">{lastSeen}</td>
                  <td className="px-5 py-3">{count}</td>
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-4">
                      <span>{confidence}</span>
                      <span className="h-1.5 w-28 overflow-hidden rounded-full bg-[#223038]"><span className="block h-full rounded-full bg-[#35d8e8]" style={{ width: confidence }} /></span>
                    </div>
                  </td>
