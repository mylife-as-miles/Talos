    ...
    <AppShell>
        ...
        <Card>
          ...
          <div className="space-y-5 p-5">
            ...
            <section>
              ...
              <div className="mt-3 space-y-2">
                {report.evidence.map((item, index) => (
                  <div key={`${item.message}-${index}`} className="rounded-lg border border-talos-line bg-black/20 p-3 text-sm">
            ...
            <section>
              ...
              {report.proposedFix.code ? <div className="mt-3"><CodeBlock value={report.proposedFix.code} /></div> : null}
            </section>
          </div>
        </Card>
        ...
        <div className="space-y-5">
          <Card>
            ...
            <dl className="grid gap-3 p-5 text-sm">
              <div><dt className="text-talos-muted">Trigger</dt><dd className="mt-1">{report.trigger}</dd></div>
              <div><dt className="text-talos-muted">Affected service</dt><dd className="mt-1">{report.affectedService}</dd></div>
              <div><dt className="text-talos-muted">Affected route</dt><dd className="mt-1">{report.affectedRoute}</dd></div>
              <div><dt className="text-talos-muted">Splunk mode</dt><dd className="mt-1">Mode: {report.splunk.mode.toUpperCase()}</dd></div>
              <div><dt className="text-talos-muted">Anomaly score</dt><dd className="mt-1">{report.anomaly.score} · {report.anomaly.level}</dd></div>
            </dl>
          </Card>
          <Card>
            ...
            <div className="space-y-3 p-5">
              {report.timeline.map((item, index) => (
                <div key={`${item.timestamp}-${index}`} className="border-l border-talos-bronze/50 pl-4">
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-2">
        <Card>
          ...
          <div className="p-5"><CodeBlock value={report.splunk.queryUsed} /></div>
        </Card>
        <Card>
          <CardHeader title="Raw Event JSON" />
          <div className="p-5"><CodeBlock value={JSON.stringify(event, null, 2)} /></div>
        </Card>
      </div>
    </AppShell>
  );
